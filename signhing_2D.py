...
like = {
    'A': likelihoods().from_sens_spec(0.85,0.9),
    'B': likelihoods().from_sens_spec(0.875,0.75)
}

AB = compute_ppv(like['A'].PLR,compute_ppv(like['B'].PLR,p))
AnB = compute_ppv(like['A'].PLR,compute_npv(like['B'].NLR,p))
nAB = compute_npv(like['A'].NLR,compute_ppv(like['B'].PLR,p))
nAnB = compute_npv(like['A'].NLR,compute_npv(like['B'].NLR,p))
...
for _ in tqdm(range(1000)): 
    ...
    a,b,c,d = 0,0,0,0
    for i,j in zip(dataset['A'],results):
        if i and j:
            a += 1
        elif i and not j:
            b += 1
    ...
    c_boxes['B-PLR'].append(CB.likelihood_ratios['B'].PLR)
    c_boxes['B-NLR'].append(CB.likelihood_ratios['B'].NLR)
    c_boxes['B-sens'].append(CB.likelihood_ratios['B'].sensitivity)
    c_boxes['B-spec'].append(CB.likelihood_ratios['B'].specificity)
    ...
    test = pd.DataFrame(
        ...
        index = ['AB','AnB','nAB','nAnB']
    )

    predictions = CB.predict(test,p = p)

    c_boxes['AB'].append(predictions[0])
    c_boxes['AnB'].append(predictions[1])
    c_boxes['nAB'].append(predictions[2])
    c_boxes['nAnB'].append(predictions[3])
...
fig_A.tight_layout()
fig_A.savefig('A_2D.png')

fig_B, Bx_B = plt.subplots(2,2)
singh(c_boxes['B-PLR'],like['B'].PLR,fig=fig_B,ax=Bx_B[0,0])
Bx_B[0,0].set_title('B - PLR')
singh(c_boxes['B-NLR'],like['B'].NLR,fig=fig_B,ax=Bx_B[0,1])
Bx_B[0,1].set_title('B - NLR')
singh(c_boxes['B-sens'],like['B'].sensitivity,fig=fig_B,ax=Bx_B[1,0])
Bx_B[1,0].set_title('B - Sensitivity')
singh(c_boxes['B-spec'],like['B'].specificity,fig=fig_B,ax=Bx_B[1,1])
Bx_B[1,1].set_title('B - specificity')
fig_B.tight_layout()
fig_B.savefig('B_2D.png')
#%%
fig2, ax2 = plt.subplots(2,2)
singh(c_boxes['AB'],AB,fig=fig2,ax=ax2[0,0])
ax2[0,0].set_title(r"$A, B$")
singh(c_boxes['AnB'],AnB,fig=fig2,ax=ax2[0,1])
ax2[0,1].set_title(r'$A, \neg B$')
singh(c_boxes['nAB'],nAB,fig=fig2,ax=ax2[1,0])
ax2[1,0].set_title(r'$\neg A, B$')
singh(c_boxes['nAnB'],nAnB,fig=fig2,ax=ax2[1,1])
ax2[1,1].set_title(r'$\neg A, \neg B$')
fig2.tight_layout()
fig2.savefig('singhing_2D.png')
